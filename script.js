...
function displayEmojees(query){
    emoje_container.innerHTML = ""; // clear previous results
    let filteredData = emojiList.filter(e=>{
        if(e.description.indexOf(query) !== -1){ // corrected comparison operator
            return true;
        }
    });
    filteredData.forEach(e=>{ // changed from emojiList to filteredData
...
window.addEventListener('load', () => displayEmojees("")); // added initial call with an empty string
